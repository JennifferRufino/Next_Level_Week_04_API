...
describe("SendMail", () => {
  ...
  it("Should not be able to send mail if user does not exists", async () => {
    const response = await request(app)
      .post("/sendMail")
      .send({ email: "jenniffer@gmail.com", survey_id: "dasdad-dasdd-dsa" });
  ...
  it("Should not be able to send mail if survey does not exists", async () => {
    await request(app)
      .post("/users")
      .send({ email: "jenniffertest@gmail.com", name: "Mikael Boff" });

    const response = await request(app)
      .post("/sendMail")
      .send({ email: "jenniffertest@gmail.com", survey_id: "dasdad-dasdd-dsa" });
